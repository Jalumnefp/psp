...
public class QuadreCites {
	...
	public QuadreCites() {
		...
		for (int i=0; i<2; i++) {
			...
			for (int j=0; j<4; j++) {
				...
				hores[k++] = String.valueOf(hora)  
						+":"+ (str_min.equals("0") ? str_min + "0" : str_min);
				min += 15;   //Sumem 15 min. per a la pròxima iteració.
			}//for (min)
			hora++;   //Una vegada recorreguts tots els cuarts de la primera hora, passem a la següent.
		}//for (hora)
		
		//Inicialitza vctor de torns a "Ningu", vol dir que no hi ha ningu al torn assignat
	...
	//-----------------------------------------
	public void assignaCitaAQuadre(String nomPacient, int i) {	//Passant-li un pacient i la posició, el guarda en el vector
